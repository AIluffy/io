export default function oinAccessPlugin({ types: t }) {
  function isOinImport(path, state) {
    const node = path.node;
    if (!t.isImportDeclaration(node)) return;
    if (node.source.value !== '@oin/store') return;
    for (const spec of node.specifiers) {
      if (t.isImportSpecifier(spec) && spec.imported.name === 'oin') {
        state.oinNames.add(spec.local.name);
      }
    }
  }

  function isOinCallee(node, state) {
    return t.isIdentifier(node) && state.oinNames.has(node.name);
  }
  ...
  return {
    name: 'oin-access-chain',
    visitor: {
      Program: {
        enter(_path, state) {
          state.oinNames = new Set();
          state.oinBindings = new Set();
            ...
            t.variableDeclaration('const', [
              t.variableDeclarator(
                ...
                t.callExpression(
                  ...
                  [t.stringLiteral('@oin/store/internal')]
                )
              ),
            ])
          );
      ...
      ImportDeclaration(path, state) {
        isOinImport(path, state);
      },
      VariableDeclarator(path, state) {
        const { id, init } = path.node;
        if (!t.isIdentifier(id) || !init) return;
        if (t.isCallExpression(init) && isOinCallee(init.callee, state)) {
          state.oinBindings.add(id.name);
        }
      },
      AssignmentExpression(path, state) {
        const { left, right } = path.node;
        if (!t.isIdentifier(left)) return;
        if (t.isCallExpression(right) && isOinCallee(right.callee, state)) {
          state.oinBindings.add(left.name);
        }
      },
      MemberExpression(path, state) {
        if (!state.oinBindings.size) return;
        ...
        if (t.isIdentifier(root) && !state.oinBindings.has(root.name)) return;
        if (t.isCallExpression(root) && !isOinCallee(root.callee, state)) return;
