---
title: "useInfiniteQuery"
description: "Function API reference."
---


## When to Use

Use this function when you need this behavior from the package API.

## Signature

```ts
<TData, TError = Error, TPageParam = unknown, TSelected = InfiniteData<TData, TPageParam>>(options: IoUseInfiniteQueryOptions<TData, TError, TPageParam, TSelected>) => IoSolidInfiniteQueryResult<TData, TError, TPageParam, TSelected>
```

## Parameters

| Name | Type |
| --- | --- |
| options | `IoUseInfiniteQueryOptions<TData, TError, TPageParam, TSelected>` |

## Returns

`IoSolidInfiniteQueryResult<TData, TError, TPageParam, TSelected>`

## Minimal Example

```ts
import { useInfiniteQuery } from '@iostore/solid';

const result = useInfiniteQuery(/* ...args */);
console.log(result);
```

## Common Pitfalls

- Validate argument types and nullability before calling.
- Confirm import path and runtime environment (server/client) expectations.

## Related APIs

- [useIO](../use-io/)
- [useIOSelector](../use-ioselector/)
- [useMutation](../use-mutation/)
- [useQuery](../use-query/)

## Source

- `packages/io-solid/src/lib/use-infinite-query.ts`
