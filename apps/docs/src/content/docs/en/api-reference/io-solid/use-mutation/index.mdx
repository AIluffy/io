...
```ts
<TData, TVariables, TError = Error, TContext = unknown>(options: IoMutationOptions<TData, TVariables, TError, TContext>) => IoSolidMutationResult<TData, TVariables, TError>
```
...
```ts
import { useMutation } from '@iostore/solid';
