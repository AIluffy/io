...
<section class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
  ...
  <div class="mt-6 flex flex-col gap-3 sm:flex-row">
    <input
      value={$state.query}
      on:input={(event) => store.query.set(event.currentTarget.value)}
      on:keydown={(event) => event.key === 'Enter' && fetchDetail()}
  ...
  <div class="mt-6">
    ...
    {#if $state.status !== 'loading' && $state.status !== 'error' && !$state.detail}
      <div
        class="rounded-xl border border-dashed border-slate-200 px-4 py-6 text-sm text-slate-500"
      >
    ...
    {#if $state.detail && $state.status !== 'loading' && $state.status !== 'error'}
