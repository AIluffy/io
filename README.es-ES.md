

# Io

Gestión de estado reactivo de grano fino para React, Lynx, Vue, Svelte y Solid.

Idioma: Inglés | [简体中文](./README.zh-CN.md)

## Características

- Actualizaciones de grano fino: solo se notifican los nodos afectados.
- Operaciones en profundidad en objetos y arrays con una API consistente.
- Registros de actualizaciones con soporte para reproducción y deshacer.
- Un modelo de estado compartido entre múltiples frameworks.
- Runtime de consultas/recursos: caché, deduplicación, invalidación, reintentos, cancelación y precarga.

## Instalación

```bash
npm i @iostore/store
```

Las APIs de consulta se exportan a través de `@iostore/store/query`.

Instala los adaptadores según sea necesario:

```bash
npm i @iostore/react
npm i @iostore/lynx
npm i @iostore/vue
npm i @iostore/svelte
npm i @iostore/solid
```

## Inicio Rápido

```ts
import { io } from '@iostore/store';
import { derived } from '@iostore/store/derived';

const state = io({ count: 0, items: [1, 2, 3] });
state.count.set((v) => v + 1);
state.items.push(4);

const doubled = derived([state.count], (count) => count * 2);
```

Reproducir y deshacer actualizaciones:

```ts
import { applyUpdate, replay, undoUpdate } from '@iostore/store/patches';

const updates = [];
state.subscribeUpdate((u) => updates.push(u));

replay(anotherState, updates);
applyUpdate(state, undoUpdate(updates[0]));
```

## Integración con Frameworks

React:

```tsx
import { useIO, useQuery } from '@iostore/react';
const value = useIO(countUnit);
const user = useQuery({
  key: ['user', id],
  queryFn: async ({ signal }) =>
    fetch(`/api/users/${id}`, { signal }).then((r) => r.json()),
});
```

Lynx:

```tsx
import { useIO, useQuery } from '@iostore/lynx';
const value = useIO(countUnit);
const user = useQuery({
  key: ['user', id],
  queryFn: async ({ signal }) =>
    fetch(`/api/users/${id}`, { signal }).then((r) => r.json()),
});
```

Vue:

```ts
import { ioRef, useIO, useQuery } from '@iostore/vue';
const stateRef = useIO(source);
const countRef = ioRef(countUnit);
const user = useQuery({
  key: ['user', id],
  queryFn: async ({ signal }) =>
    fetch(`/api/users/${id}`, { signal }).then((r) => r.json()),
});
```

Svelte:

```ts
import { createQueryStore, toReadable, toWritable } from '@iostore/svelte';
const readable = toReadable(state);
const writable = toWritable(unit);
const user = createQueryStore({
  key: ['user', id],
  queryFn: async ({ signal }) =>
    fetch(`/api/users/${id}`, { signal }).then((r) => r.json()),
});
```

Solid:

```tsx
import { useIO, useQuery } from '@iostore/solid';
const value = useIO(countUnit);
const user = useQuery({
  key: ['user', id],
  queryFn: async ({ signal }) =>
    fetch(`/api/users/${id}`, { signal }).then((r) => r.json()),
});
```

## Paquetes

- `@iostore/store`
- `@iostore/react`
- `@iostore/lynx`
- `@iostore/vue`
- `@iostore/svelte`
- `@iostore/solid`
- `@iostore/devtools`
- `@iostore/devtools-react`
- `@iostore/skill`

## Documentación

- Sitio de documentación (zh-CN): `apps/docs/src/content/docs`
- Sitio de documentación (en): `apps/docs/src/content/docs/en`

## Licencia

MIT
