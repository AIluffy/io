...
function defaultPerfSummary(windowSize: number): OinDevtoolsPerfSummary {
  return {
    windowSize,
    avgTotalMs: 0,
    maxTotalMs: 0,
...
function asTarget(target: unknown): OinDevtoolsTarget {
  const t = target as Partial<OinDevtoolsTarget>;
  if (
    typeof t?.snapshot !== 'function' ||
    typeof t?.subscribeUpdate !== 'function'
  ) {
    throw new Error(
      'createOinDevtools: target must implement snapshot() and subscribeUpdate()'
    );
  }
  return target as OinDevtoolsTarget;
}

function patchToDiff(patch: OinPatch): OinPatchDiff {
  if (patch.op === 'set')
...
export function createOinDevtools(
  targetInput: unknown,
  options?: OinDevtoolsOptions
): OinDevtools {
  const target = asTarget(targetInput);
  const createId = createIdFactory(options?.name ?? 'oin');
  ...
  const history: OinHistoryEntry[] = [];
  const errors: unknown[] = [];
  const listeners = new Set<(event: OinDevtoolsEvent) => void>();
  ...
  const getState = (): OinDevtoolsState => {
  ...
  const pushPerf = (sample: OinDevtoolsPerfSample) => {
  ...
  const appendHistory = (update: OinUpdate) => {
  ...
  const onUpdate = (update: OinUpdate) => {
  ...
  const errorHandler: OinErrorHandler = (error, path, operation) => {
    if (!enabled) return;
    errors.push(error);
    emit({
      type: 'error',
  ...
  const connectReduxDevToolsExtension = (bridgeOptions?: {
    ...
    const onBridgeUpdate = (event: OinDevtoolsEvent) => {
      if (event.type !== 'mutation') return;
      const index = history.length - 1;
      const actionType = `OIN/${index + 1}`;
    ...
    const unsub = ((): Unsubscribe => {
      const fn = (event: OinDevtoolsEvent) => onBridgeUpdate(event);
      listeners.add(fn);
    ...
    const bridge: OinDevtoolsBridge = {
      disconnect: () => {
        try {
          unsub();
