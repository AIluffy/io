import type { OinDevtoolsExportOptions } from './types.js';
import type { OinPath } from './types.js';

type InternalOptions = Required<OinDevtoolsExportOptions>;
...
export function sanitizeForJson(value: unknown, options?: OinDevtoolsExportOptions): unknown {
  const o: InternalOptions = { ...defaultOptions, ...(options ?? {}) };

  const walk = (path: OinPath, v: unknown, depth: number): unknown => {
