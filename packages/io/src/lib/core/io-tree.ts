...
type TreeScopeNode = IoTreeScope<Record<string, unknown>>;
type TreeArrayNode = IoTreeArrayUnit<unknown>;
type TreeNode = IoUnit<unknown> | TreeScopeNode | TreeArrayNode;
...
function emitScopeUpdate(state: TreeScopeState, update: IoUpdate): void {
...
function emitArrayUpdate(state: TreeArrayState, update: IoUpdate): void {
  notifyUpdate(state.updateListeners, update);
}
...
function createTreeScope(
...
): IoTreeScope<Record<string, unknown>> {
  ...
  return scope as unknown as IoTreeScope<Record<string, unknown>>;
}
...
function createTreeArray(
  ...
  const array = function (): unknown[] {
    return snapshot();
  } as unknown as IoTreeArrayUnit<unknown> & object;
  const proxy = new Proxy(array as IoTreeArrayUnit<unknown> & object, {
