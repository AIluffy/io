...
globalThis.__IO_DEVTOOLS__ = false;
...
async function loadIo() {
  try {
    ...
    const mod = await jiti.import('./src/lib/io.ts');
    if (typeof mod.io !== 'function') throw new Error('Invalid io export');
    return mod.io;
  }
}
...
for (let i = 0; i < 500; i += 1) io(fixture);
const node = io(fixture);
node.snapshot?.();
