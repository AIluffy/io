import type { OinUnsubscribe } from './types.js';

type Dependency = {
  subscribe: (fn: (...args: unknown[]) => void) => OinUnsubscribe;
};
...
export function effect(fn: () => void | (() => void)): OinUnsubscribe {
...
export class SignalState<T> implements Dependency {
  ...
  subscribe(fn: () => void): OinUnsubscribe {
