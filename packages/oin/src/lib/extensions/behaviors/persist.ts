...
function resolveStorage(custom?: OinStorageLike): OinStorageLike | null {
  if (custom) return custom;
  if (typeof localStorage !== 'undefined') return localStorage;
  return null;
}

export function persist<T>(options: PersistOptions): OinBehavior<T> {
  return (view) => {
